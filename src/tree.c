...
int print_tree(tnode *s) {  
  ...
  if (s!=NULL) {
	...
	printf("\n\n Conjunction of clauses in the Usable:\n");
      ...
      break;	  
    case CONSTANT:
      if (s->id == CTRUE) printf("TRUE (id:%d,vn:%u,md:%d,ml:%d,pol:%d,pos:%p",s->id,s->value_number,s->mdepth,s->mlevel,s->polarity,s);
      else if (s->id == CFALSE) printf("FALSE (id:%d,vn:%u,md:%d,ml:%d,pol:%d,pos:%p",s->id,s->value_number,s->mdepth,s->mlevel,s->polarity,s);
      else if (s->id == CSTART) printf("START (id:%d,vn:%u,md:%d,ml:%d,pol:%d,pos:%p",s->id,s->value_number,s->mdepth,s->mlevel,s->polarity,s);
      if (s->parent != NULL)
    ...
    case PROP: 
      printf("PROPOSITION: ");
      print_prop(s->id);
      printf(" (id:%d,vn:%u,md:%d,ml:%d,pol:%d,pos:%p",s->id,s->value_number,s->mdepth,s->mlevel,s->polarity,s);
      if (s->parent != NULL)
	printf(",parent vn:%u)\n", s->parent->value_number);
      else printf(")\n");
      return 1;
    ...
    case BOX:
      printf("MODAL: ");
      print_agent(s->id);
      printf(" (id:%d,vn:%u,md:%d,ml:%d,pol:%d,pos:%p,numdiamond:%d",s->id,s->value_number,s->mdepth,s->mlevel,s->polarity,s,s->numdiamond);
      if (s->parent != NULL)
	printf(",parent vn:%u)\n", s->parent->value_number);
      else printf(")\n");
      indentation += tab;
