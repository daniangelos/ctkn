...
extern void add_renaming(int id, tnode *formula, int set, int modal_level);
extern void add_definition(int id, tnode *formula, int set, int modal_level);
...
formulalist *get_snf_list(int type, int set, int modal_level, formulalist *l);

tnode *get_snf_modal(int type, int set, int modal_level, tnode *t) {
  int ml;
  ...
  if (t->left->type == CONJUNCTION || t->left->type == DISJUNCTION) {
    t->left->list = get_snf_list(t->left->type,set,ml,t->left->list);
    int id = rename_formula(t->left,HIGH,set,ml);
    t->left->type = PROP;
    t->left->id = id;
    ...
    t->left = get_snf_modal(t->left->type,set,ml,t->left);
    int id = rename_formula(t->left,HIGH,set,ml);
    t->left->type = PROP;
    t->left->id = id;
    t->left->left = free_tree(t->left->left);
...
formulalist *get_snf_list(int type, int set, int modal_level, formulalist *l) {
  ...
  while (aux != NULL) {
    if (aux->formula->type == CONJUNCTION || aux->formula->type == DISJUNCTION) {
      aux->formula->list = get_snf_list(aux->formula->type,set,modal_level,aux->formula->list);
      if ((type == DISJUNCTION && aux->formula->type == CONJUNCTION)) {
	int id = rename_formula(aux->formula,HIGH,set,modal_level);
	...
	clausesize = clausesize +  1; // for the new symbol
      ...
      aux->formula = get_snf_modal(aux->formula->type,set,modal_level,aux->formula);
...
void create_clause_snf(int set, int type, int modal_level, int agent, tnode *left, literal_list *right, justification *just) {
...
void create_clause (int set, tnode *s) {
  ...
  if (s->type == IMPLICATION) {
    if (s->left->type == CONSTANT && s->left->id == CSTART) {
      ...
      create_clause_snf(set,INITIAL,1,PROP,NULL,dis,just);
    }
    else if (s->left->type == CONSTANT && s->left->id == CTRUE && s->right->type != BOX && s->right->type != DIAMOND) {
      literal_list *dis = get_literal_list(s->right->list);
      create_clause_snf(set,LITERAL,1,PROP,NULL,dis,just);
    }
    else {
	...
	right = get_disjunction(s->right->left);
      ...
      create_clause_snf(set,type,1,agent,left,right,just);
    }
  }
}
...
int get_snf(int set, int modal_level, tnode *s) {
  if (s == NULL)
    return 0;
  switch (s->type) {
    case SETC:
	...
	get_snf(s->id,modal_level,s->right);
	
      }	
      break;
  case SETF:
      {
	get_snf(s->id,modal_level,s->left);
	get_snf(s->id,modal_level,s->right);
      }	
      break;
  case CONJUNCTION:
  ...
  case CONSTANT:
    {
      ...
      if (global || !ires)
	create_clause_snf(set,LITERAL,modal_level,PROP,NULL,dis,just);
      else
	create_clause_snf(set,INITIAL,modal_level,PROP,NULL,dis,just);
      ...
      get_snf(set,modal_level,newimp);
  ...
  case IMPLICATION:
	...
	s->right->list = get_snf_list(s->right->type,set,modal_level,s->right->list);
	int id;
	if (s->left->type == NEGATION)
	  id = -s->left->left->id;
	else id = s->left->id;
	if (s->right->type == CONJUNCTION) {
	  add_renaming(id,s->right,set,modal_level);
	  formulalist *aux = s->right->list;
	  while (aux != NULL) {
	    add_definition(id,aux->formula,set,modal_level);
	    aux = aux->next;
	...
	if (clause_type == MNEGATIVE && (is_literal(s->right->left) == CFALSE)) { // simplifies l => <> false
	  ...
	  just->parents = NULL;
	  just->literals = NULL;
	  create_clause_snf(set,LITERAL,modal_level,PROP,NULL,newlist,just);
	  ...
	  create_clause_snf(set,clause_type,modal_level,s->right->id,s->left,new,just);
	  free(new);
	}
	else { // there is a complex formula in the scope of the modal operator; rename the formula
	  s->right = get_snf_modal(BOX,set,modal_level,s->right);
	  get_snf(set,modal_level,s);
	}
      }
      else {
	if (is_literal(s->right)) {
	  ...
	  just->parents = NULL;
	  just->literals = NULL;
	  create_clause_snf(set,LITERAL,modal_level,PROP,NULL,new1,just);
