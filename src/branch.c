...
void add_mclause(tclause* clause, int type);
...
int add_literal(int literal, literalshash** list);
...
void branch_mclauses(hml_mclauses *mclauses){
    ...
    for (maux=mclauses; maux != NULL; maux=maux->hml.next){
        for(hag = maux->id; hag != NULL; hag=hag->hid.next) {
            ...
            if(pos != NULL){
                for(hmaxlit = pos->lit; hmaxlit != NULL; 
                        hmaxlit = hmaxlit->hmax.next) {
                    clauseslist *l = hmaxlit->list;
                    while(l != NULL) {
                        tclause *clause = get_clause(l->clause);
                        add_mclause(clause, MPOSITIVE);
                        l = l->next;
                    }
                }
            }
            ...
            if(neg != NULL){
                for(hmaxlit = neg->lit; hmaxlit != NULL; 
                        hmaxlit = hmaxlit->hmax.next) {
                    clauseslist *l = hmaxlit->list;
                    while(l != NULL) {
                        tclause *clause = get_clause(l->clause);
...
void add_pclause(tclause* clause){
    ...
    if(mlbranches == NULL) {
        while(literals != NULL){
            tbranch *branch = create_branch(clause->modal_level, agent);
            /*branch->litmin = branch->litmax = literals->literal;*/
    ...
    for(idaux=agaux->branches; idaux!=NULL; idaux=idaux->hid.next){
        ...
        while(literals->next != NULL){
            ...
            if(add_literal(literals->literal, &provlist) < 0)
                ...
                newbranch->tliterals = branch->tliterals + 1;
                /*add_branch(newbranch);*/
                add_brancheslist(&newbranch, &add);
            }
            literals = literals->next;
        }
        if(add_literal(literals->literal, &(branch->literals))){
            ...
            /*printf("inserindo literais a branches existentes: \n");*/
            /*printf("%d\n", a);*/
            if(a){
                delete_branch(&branch);
            }
...
void add_mclause(tclause *clause, int type){
    ...
    HASH_FIND(htp, ml->types, &type, sizeof(int), t);
    if(t == NULL){
        t = malloc(sizeof(htype));
        t->type = type;
        t->heads = NULL;
        HASH_ADD(htp, ml->types, type, sizeof(int), t);
    }
    ...
    add_literal(clause->right->literal, &(l->literals));
}
...
void create_empty_ml(int modal_level, int literal, int agent){
    ...
    add_literal(literal, &(branch->literals));
    add_branch(branch);
}
...
void add_branch(tbranch *branch){
    ...
    if(bid == NULL){
        /*printf("Inserindo branches novos:\n");*/
...
void print_branch(){
    ...
    for(mlit = mlliterals; mlit != NULL; mlit = mlit->hml.next){
        printf("[%d]: \n", mlit->ml);
        for(hlit = mlit->types; hlit != NULL; hlit = hlit->htp.next){
            printf("\t");
