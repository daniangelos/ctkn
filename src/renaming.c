...
extern void sort_formulalist(formulalist **list);
...
int add_renaming (int id, tnode *formula, int set, int modal_level);
...
extern int get_snf(int set, int modal_level, tnode *s);

void add_definition(int id, tnode *formula, int set, int modal_level) {
  prop_node *p;
...
int insert_renaming_table(ren_key key, tnode *formula, int type, int set, int modal_level) {
  ...
  if (c != NULL) {
	...
	if (!aux->definition_added) {
	  add_definition(aux->renamed_by,aux->formula,set,modal_level);
    ...
    add_definition(p->id,formula,set,modal_level);
    return p->id;
  }
  else {
...
int rename_formula(tnode *formula, int type, int set, int modal_level) {
  int id = 0;
  
  if (normal_renaming) {
    prop_node *p = insert_pnew_node(newtemp++,type);
    id = p->id;
    add_definition(id,formula,set,modal_level);
  }
  else {
    ren_key key;  
    key.ml = modal_level;
    ...
    id = insert_renaming_table(key,formula,type,set,modal_level);
    ...
    if (strong_renaming) {
      ...
      not = get_nnf(not);
      not = tree_hash(not);
      add_renaming(-id,not,set,modal_level);
      not = free_tree(not);
    }
  }
  return id;
