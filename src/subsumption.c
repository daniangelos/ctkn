...
/*
    Subsumption is only performed at the same modal level of clause 
*/
...
void forward_subsumes (tclause *clause, hml_pclauses **set) {
  ...
  if (hmlp != NULL) {
	...
	while (hsc != NULL) {
	  if ((clause)->size <= hsc->size) {
	    ...
	    while (haux != NULL) {
	      if (haux->clause > (clause)->number) {
		tclause *aux = get_clause(haux->clause);
  ...
  while (todelete != NULL) {
    ...
    tclause *aux = get_clause(todelete->clause);
