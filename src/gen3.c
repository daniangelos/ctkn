...
void try_gen3(tclause *chosen, tclause *neg, listsclauseslist *resolvents) {
  ...
  while (aux != NULL) {
    ...
    parents->next->clause = neg->number;
	...
	if (try) {
	  ...
	  if (htc != NULL) {
	    ...
	    for (hmaxlit = htc->lit; hmaxlit != NULL; hmaxlit=hmaxlit->hmax.next) {
	      ...
	      while (aux_neg != NULL) {
		tclause *negative_clause = get_clause(aux_neg->clause);
