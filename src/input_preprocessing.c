...
tnode *calculate_polarity (tnode *s, int sign, int ml) {
  ...
  if (s->type == PROP) {
    ...
    if (sign > 0) {
      p->occur_positive++;
      p = insert_hml_props(POSITIVE,ml,p,s);
    }
    else if (sign < 0) {
      p->occur_negative++;
      p = insert_hml_props(NEGATIVE,ml,p,s);
    }
  }
...
int propagate_falsity(tnode **node, tnode **nodeparent) {
  ...
  if (position != NULL && parent != NULL) {
	...
	if(find_p_position(position))
	  printf("\n We have a problem, Houston! Falsity Propagation type, id, vn, position || parent type id vn: %d %d %u %p|| %d %d %u",position->type,position->id, position->value_number, position,position->parent->type,position->parent->id,position->parent->value_number);
      }
    }
  }
  //  print_out("FALSE");
...
int propagate_truth(tnode **node, tnode **parentnode) {
  ...
  if (position != NULL && parent != NULL) {
	...
	if(find_p_position(position))
	  printf("\n We have a problem, Houston! Truth Propagation type, id, vn, position || parent type id vn: %d %d %u %p || %d %d %u",position->type,position->id, position->value_number, position, position->parent->type,position->parent->id,position->parent->value_number);
      }
    }
  }
  //  print_out("TRUTH");
	...
	HASH_ITER(hml,p->hmlpropsposition,hmlpositions,thmlpos) {
	  if (hmlpositions->ml_occur_positive == 0 && hmlpositions->ml_occur_negative == 0) {
	    ...
	    hmlpositions = NULL;
		   ...
		   (early_mlple == ON && hmlpositions->ml_occur_positive != 0 &&  hmlpositions->ml_occur_negative == 0)) {
	    flag = 1;
...
tnode *input_preprocessing(tnode *root) {
  ...
  if (linearise) {
    root = eliminate_double_implications(root,1,1);
    print_out("Linearisation");
  }
    
  if (early_ple == ON || early_mlple == ON) {
    if (!linearise) {
      root = calculate_polarity(root,1,1);
      print_out("Polarity");
    }
    do_early_ple();
