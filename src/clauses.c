...
extern void print_short(int cycle);
...
clauseslist *remove_clause_list(tclause *c, clauseslist *l) {
  ...
  if (l == NULL) return l;
  while (aux != NULL && aux->clause != c->number) {
    previous = aux;
    aux = aux->next;
  }
  ...
  if (hml != NULL) {
	...
	HASH_ITER(hmax,htc->lit,hmaxlit,hmaxlitaux) {
	  ...
	  while (aux != NULL) {
	    ...
	    deleted->parents->clause = clause->number;
	    deleted->parents->next = malloc(sizeof(clauseslist));
	    deleted->parents->next->clause = c->number;
	    ...
	    just1->parents->clause = clause->number;
	    just1->parents->next = malloc(sizeof(clauseslist));
	    just1->parents->next->clause = c->number;
...
int compare_clauses(clauseslist *left, clauseslist *right) {
  tclause *c1 = get_clause_x(left->clause);
  tclause *c2 = get_clause_x(right->clause);
...
clauseslist *sorted_merge_clauses(clauseslist *left, clauseslist *right) {
  clauseslist *result = NULL;
  if (left == NULL)
    return right;
  else if (right == NULL)
    return left;
  else if (compare_clauses(left,right)){
    result = left;
    result->next = sorted_merge_clauses(left->next,right);
  }
  else {
    result = right;
    result->next = sorted_merge_clauses(left,right->next);
  }
  return result;
}
...
clauseslist *construct_proof(tclause *proof) {
  clauseslist *first = NULL;

  if (proof != NULL) {
    first = malloc(sizeof(clauseslist));
    first->clause = proof->number;
    first->next = NULL;

    clauseslist *last = first;

    if (proof->just != NULL) {
      clauseslist *parents = proof->just->parents;
      while (parents != NULL) {
	tclause *clause = get_clause_x(parents->clause);
	clauseslist *new = construct_proof(clause);
	last->next = new;
...
void print_all_proof(tclause *proof) {
  clauseslist *clauses = construct_proof(proof);
  sort_clauses(&clauses);
  remove_repeated_clauses(&clauses);
  clauseslist *aux = clauses;
  while (aux != NULL) {
    tclause *c = get_clause_x(aux->clause);
    print_clause(c);
    clauseslist *tmp = aux;
    aux = aux->next;
    free(tmp);
...
void insert_clause_deleted(tclause *clause) {
  if (clause != NULL) {
    ...
    if (clause->just->rule == SUBSUMED || clause->just->rule == REPEATED) {
