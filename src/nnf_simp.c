...
tnode *get_nnfsimp (tnode *s) {
  ...
  switch (s->type) {
	...
	return s;
      ...
      break;
    case NEGATION: // This is not needed if it is in NNF, but if early ple is run before nnf simplication, we save a call to get nnf if we introduce double negation removal and NNF of negated constants here.
      {
	if (s->left != NULL) {
